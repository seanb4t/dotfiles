...
- **Switching**: `dot_local/bin/executable_ghostty-mux` dispatches on `~/.local/state/mux`.
...
- catppuccin/tmux and extrakto — pinned to immutable commits/tags.
- The herdr skill is pinned in `agent-skills.json` and restored with `npx skills`.
...
The architecture guidance is now in this repo-local AGENTS.md; CLAUDE.md imports it. Instruction routing:

| Scope | File |
|-------|------|
| Global coding rules, every session | shared `.chezmoitemplates/user-preferences.md.tmpl` rendered for both agents |
| This repo's architecture | `AGENTS.md` (here, not deployed) |
| Conditional, tool-specific | a skill installed by `npx skills` into `~/.agents/skills`; GSD remains GSD-managed |
...
- `chezmoi apply` stops on script failure — use `chezmoi apply --exclude=scripts ~/.config/fish/` to apply specific paths without triggering lifecycle scripts (e.g., brew bundle).
...
See `docs/agent-setup.md` for restore commands, configuration ownership, and parity checks.
Non-GSD skills are restored with `scripts/agent-skills.py`; GSD exclusively owns `gsd-*` skills and its runtime.
